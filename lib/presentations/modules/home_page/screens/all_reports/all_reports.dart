...
import 'package:pronight_vendor/data/models/response/reports_model.dart';
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                SizedBox(width: 284.w,height: 36.h,
                    child: CustomTextFormField(controller: provider.searchController,underLineColor: Colors.transparent,
                      borderRaduis: 7.r,
              ...
              Expanded(
                child: Padding(
                  ...
                  child: AnimationLimiter(
                    child: data.isLoading?const AppLoader():
                    (data.reportsModel?.data?.isEmpty==true)?const NoDataScreen():
                    ListView.builder(
                        shrinkWrap: true,
                        itemCount: data.reportsModel?.data?.length,
                        itemBuilder: (context, index) {
                         OneReport? model =data.reportsModel?.data?[index];
                          return AnimationConfiguration.staggeredGrid(
                              ...
                              child:  ScaleAnimation(
                                  ...
                                  InkWell(onTap: (){
                                    NavigatorHandler.push(ReportDetailsScreen(model: model,));
                                  },
                                    child: Stack(
                                      children: [
                                                ...
                                                child: Column(mainAxisSize: MainAxisSize.max,
                                                  ...
                                                  children: [
                                                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                      children: [
                                                        CustomText(title: model?.unit?.title??'',fontColor: AppColors.textColor4),
                                                        Row(
                                                          children: [
                                                            CustomSvgIcon(assetName: AppAssets.address,width: 18.w,height: 18.h,color: AppColors.textColor,),
                                                            SizedBox(width: 5.w),
                                                            CustomText(title: model?.unit?.city?.title??'',fontColor: AppColors.textColor,fontSize: AppFonts.font_10),
                                                          ],
                                                        )
                                                      ],),
                                                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                      children: [
                                                        Row(
                                                          children: [
                                                            ...
                                                            SizedBox(width: 5.w),
                                                            CustomText(title: '20 ${AppTranslate.room.tr()}',fontColor: AppColors.textColor,fontSize: AppFonts.font_10),
                                                          ],
                                                        ),
                                                        Row(
                                                          children: [
