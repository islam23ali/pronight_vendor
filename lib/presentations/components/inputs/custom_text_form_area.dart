...
class CustomTextFormFieldArea extends StatelessWidget {
  ...
  const CustomTextFormFieldArea({super.key,required this.controller,this.hint,this.prefix,this.suffix,this.readOnly = false,this.bgColor, this.onChange, this.borderRaduis, this.height, this.focusNode, this.fontSizeHint, this.fontSize});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: bgColor??AppColors.darkColor.withAlpha((0.08 * 255).round()),borderRadius: BorderRadius.circular(borderRaduis??16.r),border: Border.all(color: AppColors.darkColor.withAlpha((0.03 * 255).round()))),
      child: TextFormField(
        focusNode: focusNode,
        readOnly:readOnly ,
